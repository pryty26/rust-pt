...
# And I've contributed to that script in Arti too. \(・ω・)/
import argparse
import fnmatch
import sys
import os
...
def process(lints, fn, always_insist):
    insist = (
        always_insist
        or fnmatch.fnmatch(fn, "transports/*/src/lib.rs")
        or fnmatch.fnmatch(fn, "rust_pt/*/src/main.rs")
    )

    tmp_name = fn + ".tmp~"
    # Explicitly specify UTF-8 encoding
    # because Windows defaults to a system locale encoding (e.g., GBK or cp1252)
    outp = open(tmp_name, "w", encoding="utf-8")
    inp = open(fn, "r", encoding="utf-8")
    try:
...
def check_diff(fn: str, tmp_name: str) -> str | None:
    """
    fn = original file name
    tmp_name = temporary file name which contains the modified content
    When caller call the function,
    caller itself will check if the return value is None or not.
    If it is not None,
    caller should print the diff to stdout
    and add the file name to deferred_errors list.
    Like:
    ```python
    sys.stdout.writelines(diff)
    deferred_errors.append(fn)
    """
    with open(fn, "r", encoding="utf-8") as original, open(
        tmp_name, "r", encoding="utf-8"
    ) as modified:
...
def main(lints, files):
    if not os.path.exists("./rust_pt/pt_config/src/lib.rs"):
        print("Run this from the top level of an rust_pt repo.")
        sys.exit(1)

    always_insist = True
    if not files:
        files = [str(file) for file in Path("./").rglob("*.rs")]
        always_insist = False
